...
const KEYCLOAK_DOCKER_IMAGE_URI_MAP: dockerImageMap = {
  ...
  "aws-cn":
    "048912060910.dkr.ecr.cn-northwest-1.amazonaws.com.cn/quay/keycloak/keycloak:",
};
...
export class KeyCloak extends Construct {
  ...
  constructor(scope: Construct, id: string, props: KeyCloakProps) {
    ...
    this.applicationLoadBalancer =
      keycloakContainerService.applicationLoadBalancer;
...
export class Database extends Construct {
  ...
  private _createServerlessCluster(props: DatabaseProps): DatabaseConfig {
    const dbCluster = new rds.ServerlessCluster(
      this,
...
export class ContainerService extends Construct {
  ...
  constructor(scope: Construct, id: string, props: ContainerServiceProps) {
    ...
    const isQuarkusDistribution =
      parseInt(props.keycloakVersion.version.split(".")[0]) > 16;
    ...
    if (isQuarkusDistribution) {
      ...
      secrets = {
        ...
        KEYCLOAK_ADMIN: ecs.Secret.fromSecretsManager(
          props.keycloakSecret,
          "username",
        ),
        KEYCLOAK_ADMIN_PASSWORD: ecs.Secret.fromSecretsManager(
          props.keycloakSecret,
          "password",
        ),
      };
    ...
    cluster.addCapacity("DefaultAutoScalingGroupCapacity", {
      ...
      desiredCapacity: props.ecsAsgDesiredCapacity
        ? props.ecsAsgDesiredCapacity
        : 2,
    });
    ...
    if (props.autoScaleTask) {
      ...
      scaling.scaleOnCpuUtilization("CpuScaling", {
        targetUtilizationPercent:
          props.autoScaleTask.targetCpuUtilization ?? 75,
      });
    }
    ...
    this.applicationLoadBalancer = new elbv2.ApplicationLoadBalancer(
      ...
      {
        vpc,
        vpcSubnets: props.internetFacing
          ? props.publicSubnets
    ...
    listener.addTargets("ECSTarget", {
      ...
      stickinessCookieDuration:
        props.stickinessCookieDuration ?? cdk.Duration.days(1),
      port: containerPort,
      protocol,
    });
  ...
  private getKeyCloakDockerImageUri(version: string): string {
    return this.getImageUriFromMap(
      KEYCLOAK_DOCKER_IMAGE_URI_MAP,
      version,
      "KeycloakImageMap",
    );
  }
